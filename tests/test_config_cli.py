...
class TestConfigShow:
    ...
    def test_show_masks_api_key(self, tmp_path: Path, monkeypatch):
        ...
        with patch("skills.youtube_transcribe.transcribe.get_api_key") as mock_get:
...
class TestConfigTest:
    def test_test_configured_backend_exits_zero(self, tmp_path: Path, monkeypatch):
        ...
        with patch("skills.youtube_transcribe.transcribe.build_backend", return_value=mock_backend):
    ...
    def test_test_build_backend_exception(self, tmp_path: Path, monkeypatch):
        monkeypatch.setattr("skills.youtube_transcribe.transcribe.CONFIG_PATH", tmp_path / "config.toml")
        with patch("skills.youtube_transcribe.transcribe.build_backend", side_effect=ValueError("fail")):
            r = _runner().invoke(cli, ["config", "test", "gemini"])
        assert r.exit_code != 0
...
class TestConfigWizard:
    def test_wizard_calls_run_wizard(self, tmp_path: Path, monkeypatch):
        monkeypatch.setattr("skills.youtube_transcribe.transcribe.CONFIG_PATH", tmp_path / "config.toml")
        with patch("skills.youtube_transcribe.transcribe.run_wizard") as mock_wizard:
